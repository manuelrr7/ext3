...
import ajaxmvc.modelo.beans.Calle;
 ...
 * @version  Ajax-MVC2
 */
public class AccionCalles implements Accion {
	/**
	...
	public boolean ejecutar(HttpServletRequest request,
		...
		cod = request.getParameter("codPoblacion");
		System.out.println("cpoblacion calle "+cod);
		modelo.setContentType("application/json; charset=UTF-8");
		Calle calle = new Calle();
		
		if (procesaBD.getCalles(cod) != null){		
			
		
			
		}else{
			calle.setError("Error al cargar las calles");
		}
		json = new Gson().toJson(procesaBD.getCalles(cod));
