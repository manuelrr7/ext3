...
import ajaxmvc.modelo.beans.Cliente;
...
public class AccionRegistrar implements Accion {
	...
	public boolean ejecutar(HttpServletRequest request,
		...
		String nombre, json, ap, apl, piso, metros, calle, cod, codpo, codprov, num;
		
		cod = request.getParameter("codigo");
		
		nombre = request.getParameter("nombre");
		ap = request.getParameter("apellido");
		apl = request.getParameter("apellido2");
		calle = request.getParameter("calle");
		num  = request.getParameter("numero");
		piso = request.getParameter("piso");
		metros = request.getParameter("metros");
		codpo = request.getParameter("codigoPoblacion");
		codprov = request.getParameter("codigoProvincia");
		System.out.println("codigo clien "+cod+" cid. prov."+codprov+" cod. pobla "+codpo);
		
		modelo.setContentType("application/json; charset=UTF-8");
		Cliente cliente = new Cliente();
		if ((nombre!=null && !nombre.equals("")) && (calle!=null && !calle.equals(""))
				&& (codpo!=null && !codpo.equals("")) && (codprov!=null && !codprov.equals("")) && (num!=null && !num.equals("")))
		{
			
			cliente.setNombre(nombre);
			cliente.setApellido(ap);
			cliente.setApellido2(apl);
			cliente.setCalle(calle);
			cliente.setNumero(num);
			cliente.setPiso(piso);
			cliente.setMetros(metros);
			cliente.setCodigoPoblacion(codpo);
			cliente.setCodigoProvincia(codprov);
		if (procesaBD.registrar(cliente)){		
			
		
			
		}else{
			cliente.setError("Cliente invalido");
		}
		}else{
			cliente.setError("Debe rellenar todos los campos");
		}
