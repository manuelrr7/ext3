...
import ajaxmvc.modelo.beans.Poblacion;
 ...
 * @version  Ajax-MVC2
 */
public class AccionPoblaciones implements Accion {
	/**
	...
	public boolean ejecutar(HttpServletRequest request,
		...
		cod = request.getParameter("codigoProvincia");
		
		modelo.setContentType("application/json; charset=UTF-8");
		Poblacion poblacion = new Poblacion();
		
		if (procesaBD.getPoblaciones(cod) != null){		
			
		
			
		}else{
			poblacion.setError("Error al cargar las poblaciones");
		}
		json = new Gson().toJson(procesaBD.getPoblaciones(cod));
